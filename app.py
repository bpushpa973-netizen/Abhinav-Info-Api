...
async def create_jwt(region: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        print(f"[{region}] Content-Type:", resp.headers.get("Content-Type"))
        ...
        try:
            login = FreeFire_pb2.LoginRes()
            login.ParseFromString(resp.content)
            msg = json.loads(json_format.MessageToJson(login))
        except Exception as e:
            print(f"❌ DecodeError for {region}: {e}")
